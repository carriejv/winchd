...
/// Maximum buffer size to allocate for retrieved x properties, measured in 4-byte chunks. Equivalent to 256 UTF8/ASCII chars.
const MAX_XPROP_BYTE_SIZE: u32 = 64;
/// Size in bytes of x window ids
const WINDOW_ID_SIZE: usize = size_of::<xproto::WINDOW>();

/// Gets the WM_NAME and WM_CLASS of the currently active (selected) X window.
...
fn get_xprop_strings(xcb_conn: &XCBConnection, window: xproto::WINDOW, prop_name: &[u8]) -> Result<Vec<String>, Box<dyn Error>> {
    ...
    for split_val in prop_result.value.split(|&x| x == 0) {
        let split_string = String::from_utf8(split_val.to_vec())?;
        // WM_CLASS may contain empty class names (back-to-back null bytes), which are useless to us
        if String::is_empty(&split_string) {
            continue;
        }
        prop_vec.push(split_string);
    }
    return Ok(prop_vec);
}
